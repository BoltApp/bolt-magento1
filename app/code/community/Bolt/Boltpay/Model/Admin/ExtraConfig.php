...
class Bolt_Boltpay_Model_Admin_ExtraConfig extends Mage_Core_Model_Config_Data
{
    ...
    public function save()
    {
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
            ...
            Mage::getSingleton('core/session')->addError(static::helper()->__('Invalid JSON for Bolt Extra Options. '.json_last_error_msg()));
        }
    ...
    public function hasValidBoltPrimaryColor($hexColor) {
        if ( !($isValid = (!empty($hexColor) && preg_match('/^#(([A-Fa-f0-9]{6})|([A-Fa-f0-9]{8}))$/', $hexColor))) ) {
            Mage::getSingleton('core/session')->addError(static::helper()->__('Invalid hex color value for extra option `boltPrimaryColor`. [%s] It must be in 6 or 8 character hex format.  (e.g. #f00000 or #3af508a2)', $hexColor));
        }
        return $isValid;
    }
