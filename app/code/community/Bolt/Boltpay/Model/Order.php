...
class Bolt_Boltpay_Model_Order extends Bolt_Boltpay_Model_Abstract
{
    ...
    public function createOrder($reference, $sessionQuoteId = null, $isAjaxRequest = false, $transaction = null)
    {
        ...
        try {
            if (empty($reference)) {
                throw new Exception(static::helper()->__("Bolt transaction reference is missing in the Magento order creation process."));
            }

            $transaction = $transaction ?: static::helper()->fetchTransaction($reference);

            $immutableQuoteId = static::helper()->getImmutableQuoteIdFromTransaction($transaction);
            $immutableQuote = $this->getQuoteById($immutableQuoteId);

            if (!$sessionQuoteId){
                /** @var Bolt_Boltpay_Helper_Data $boltHelperBase */
                $boltHelperBase = static::helper();
            ...
            if ($immutableQuote->isEmpty()) {
                throw new Exception(static::helper()->__("The expected immutable quote [$immutableQuoteId] is missing from the Magento system.  Were old quotes recently removed from the database?"));
            }

            if(!$this->allowOutOfStockOrders() && !empty($this->getOutOfStockSKUs($immutableQuote))){
                throw new Exception(static::helper()->__("Not all items are available in the requested quantities. Out of stock SKUs: %s", join(', ', $this->getOutOfStockSKUs($immutableQuote))));
            }

            // check if the quotes matches, frontend only
            if ( $sessionQuoteId && ($sessionQuoteId != $immutableQuote->getParentQuoteId()) ) {
                throw new Exception(
                    static::helper()->__("The Bolt order reference does not match the current cart ID. Cart ID: [%s]  Bolt Reference: [%s]",
                        $sessionQuoteId , $immutableQuote->getParentQuoteId())
                );
            }
            ...
            if ($parentQuote->isEmpty()) {
                throw new Exception(
                    static::helper()->__("The parent quote %s is unexpectedly missing.",
                ...
                throw new Exception(
                    static::helper()->__("The parent quote %s for immutable quote %s is currently being processed or has been processed for order #%s. Check quote %s for details.",
            ...
            if ($packagesToShip) {
                ...
                if (!$shippingMethodCode) {
                    ...
                    static::helper()->collectTotals($immutableQuote);
                ...
                if ($shippingMethodCode) {
                    ...
                    $errorMessage = static::helper()->__('Shipping method not found');
                    $metaData = array(
                    ...
                    static::helper()->notifyException(new Exception($errorMessage), $metaData);
                }
            }
            //////////////////////////////////////////////////////////////////////////////////
            ...
            static::helper()->collectTotals($immutableQuote, true)->save();
            ...
            if (!$preExistingOrder->isObjectNew()) {
                ...
                if ( $preExistingTransactionReference === $reference ) {
                    static::helper()->notifyException(
                        new Exception( static::helper()->__("The order #%s has already been processed for this quote.", $preExistingOrder->getIncrementId() ) ),
                        array(),
                        'warning'
                    );
    ...
    protected function validateProducts(Mage_Sales_Model_Quote $quote)
    {
        ...
        if ($outOfStockSKUs) {
            ...
            $errorMessage = static::helper()->__("Product " .
                join(", ", $outOfStockSKUs) .
        ...
        if ($this->shouldPutOrderOnHold()) {
            ...
            $errorMessage = static::helper()->__("Please review " .
    ...
    protected function validateSubmittedOrder($order, $quote) {
        if(empty($order)) {
            ...
            throw new Exception(static::helper()->__('Order is empty after call to Sales_Model_Service_Quote->submitAll()'));
        }
    }
