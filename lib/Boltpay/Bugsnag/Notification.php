...
class Bugsnag_Notification
{
    ...
    public function postJSON($url, $data)
    {
        ...
        try {
            if (method_exists('GuzzleHttp\Client','sendAsync')) {
                $this->postWithGuzzle($url, $body);
            } elseif (ini_get('allow_url_fopen')) {
                $this->postWithFopen($url, $body);
            } else {
                error_log('Bugsnag Warning: Couldn\'t notify (neither Guzzle or allow_url_fopen are available on your PHP installation)');
            }
        } catch (Exception $e) {
     ...
     * @return void
     */
    private function postWithGuzzle($url, $body)
    {
        try {
