...
class Bolt_Boltpay_Model_UpdaterTest extends PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        $this->boltHelperMock = $this->getClassPrototype('Bolt_Boltpay_Helper_Data')
            ->setMethods(array('getApiClient', 'notifyException', 'isModuleEnabled'))->getMock();
  ...
  "draft": false
    ...
    public function addUpdateMessage_withVariousVersionCombinationsProvider()
    {
        return array(
            ...
            array(
                ...
                'expectedNotificationSeverity' => Mage_AdminNotification_Model_Inbox::SEVERITY_MINOR,
            ),
            array(
                'installedVersion'             => '1.0.0',
                'latestVersion'                => '2.1.1',
    ...
    {
        $ioFileMock = $this->getClassPrototype('Varien_Io_File')->setMethods(array('read', 'ls', 'fileExists'))->getMock();
        Bolt_Boltpay_TestHelper::setNonPublicProperty($this->currentMock, '_ioFile', $ioFileMock);
        ...
        $ioFileMock->expects($this->exactly(19))->method('fileExists')->withConsecutive(
            ...
            array('tools' . DS . 'config.ini'),
            array('docker-container' . DS . 'config.ini'),
            array('errors' . DS . 'config.ini'),
            array('media' . DS . 'config.ini'),
            array('js' . DS . 'config.ini'),
            array('skin' . DS . 'config.ini'),
            array('.git' . DS . 'config.ini'),
            array('ws4er8ty0u' . DS . 'config.ini'),
            array('tests' . DS . 'config.ini'),
            array('.github' . DS . 'config.ini'),
            array('bin' . DS . 'config.ini'),
            array('app' . DS . 'config.ini'),
            array('lib' . DS . 'config.ini'),
            array('shell' . DS . 'config.ini'),
            array('includes' . DS . 'config.ini'),
            array('.circleci' . DS . 'config.ini'),
            array('dev' . DS . 'config.ini'),
            array('drcfgybhuni' . DS . 'config.ini'),
            array('var' . DS . 'config.ini') // not called
        ...
        $ioFileMock->expects($this->exactly(2))->method('read')->withConsecutive(
            array('ws4er8ty0u' . DS . 'config.ini'),
            array('drcfgybhuni' . DS . 'config.ini')
        )->willReturnOnConsecutiveCalls(
